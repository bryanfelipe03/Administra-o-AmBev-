...
## 🆕 Atualização: Unidades por Fardo

Esta versão calcula "fardos + unidades soltas" no PDF e na mensagem do WhatsApp.
Para isso, rode no **SQL Editor** do Supabase (uma única vez):

```sql
ALTER TABLE produtos ADD COLUMN IF NOT EXISTS unidades_por_fardo INTEGER DEFAULT 1;
```

Depois disso, ao cadastrar um produto, preencha quantas unidades cabem em 1 fardo
(ex: 24 latas, 12 garrafas). Produtos com esse campo vazio/1 aparecem só como
"unidades soltas".
